...
<form class="layui-form layui-form-pane" style="padding:20px;" action="" method="post">
  ...
  <div class="layui-form-item" pane>
    <label class="layui-form-label">是否开启评论和赞</label>
    <div class="layui-input-block">
      <input type="radio" name="isopenthereview" value="0" title="不开启"  <?php  if(!$settings['isopenthereview']) { ?> checked="checked" <?php  } ?>>
      <input type="radio" name="isopenthereview" value="1" title="开启"  <?php  if($settings['isopenthereview']) { ?> checked="checked" <?php  } ?>>
    </div>
  </div>
  <div class="layui-form-item" pane>
    <label class="layui-form-label">首页模板</label>
    <div class="layui-input-block">
      <input type="radio" name="showmode" value="0" title="用户相册展示"  <?php  if(!$settings['showmode']) { ?> checked="checked" <?php  } ?>>
      <input type="radio" name="showmode" value="1" title="相册模板展示"  <?php  if($settings['showmode']) { ?> checked="checked" <?php  } ?>>
    </div>
  </div>
  <div class="layui-form-item" pane>
    <label class="layui-form-label">首页列表内容排序</label>
    <div class="layui-input-block">
      <input type="radio" name="ishot" value="0" title="时间排序"  <?php  if(!$settings['ishot']) { ?> checked="checked" <?php  } ?>>
      <input type="radio" name="ishot" value="1" title="点击排序"  <?php  if($settings['ishot']) { ?> checked="checked" <?php  } ?>>
    </div>
  </div>
  <div class="layui-form-item" pane>
...
<script type="text/javascript">
	...
	$('.js-checkremotecos').on('click', function(){
        ...
        var local = $.trim($('select[name="cos[local]"]').val());
        if (local == '') {
            util.message('请选择bucket所在区域');
            return false;
        }
